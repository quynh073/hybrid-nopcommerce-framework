package pageUIs.user;

public class UserHomePUI {
    public static final String REGISTER_LINK = "class=ico-register";
    public static final String LOGIN_LINK = "class=ico-login";
    public static final String MY_ACCOUNT_LINK = "class=ico-account";
    public static final String SEARCH_TEXTBOX = "id=small-searchterms";
    public static final String SEARCH_BUTTON = "xpath=//form[@id='small-search-box-form']//button[text()='Search']";
    public static final String TOP_MENU_LINK = "xpath=//ul[contains(@class,'top-menu')][1]//a[contains(text(),'%s')]";
    public static final String SUB_MENU_LINK = "xpath=//ul[contains(@class,'top-menu')][1]//a[contains(text(),'%s')]//following-sibling::ul//a[contains(text(),'%s')]";

    public static final String PRODUCT_LINK = "xpath=//div[@class='product-item']//a[text()='%s']";
}
